...
class AD9956(Device):
    ...
    def make_rsrrw(self, rate=1):
        ...
        Args:
            ...
            minimum time between steps is 1/SYNCCLK = 10 ns
            maximum time between steps is 2**16/SYNCCLK = 1525.879 Hz for 400 MHz SYSCLK (100 MHz SYNCCLK)
        Returns:
    ...
    def make_fsrrw(self, rate=1):
        ...
        Args:
            rate: (int) from -1 to -2**16.  if <  0, increase time between sweep steps to  delta_t = -rate / SYNC_CLK 
            minimum time between steps is 1/SYNCCLK = 10 ns
            maximum time between steps is 2**16/SYNCCLK = 1525.879 Hz for 400 MHz SYSCLK (100 MHz SYNCCLK)
        Returns:
            list of 2 bytes, specifying sweep step rate in units of SYNCCLK 
    ...
    def make_rdftw(self, rate):
        ...
        Args: 
            rate: (int) from 1 to 2**24.  If rate > 0 defines ramp step size in Hz.  delta_f = rate * SYS_CLK/2**24
            the minimum step size is SYSCLK/2**24 = 23.8418579102 Hz for a 400 MHz clock. 
            the maximum step size is SYSCLK = 400 MHz
    ...
    def set_linear_ramp(self, start=None, stop=None, rate=1):
        ...
        Args:
            ...
            rate: (int) if < 0, number of sys_clk (100 MHz) cycles per step.  1-2**16
                if > 0, step size from 1-16777216 (2**24)
        Returns:
            None
        """
        #update the frequencies for bookkeeping purposes
        self.frequency_low = start
        self.frequency_high = stop
        if rate == None:
            rate = 1
        if stop >= start:
#            print '!!!', rate
